...
class SGIConfig(BaseSettings):
    model_config = SettingsConfigDict(env_prefix='GIGA_CHAT_APP_')

    HTTP_PROTOCOL: str = Field(env="HTTP_PROTOCOL", default="http")
    HOST: str = Field(env="HOST", default="0.0.0.0")
    PORT: int = Field(env="PORT", default=8082)
    ...
    WSGI_APP: str = Field(env="WSGI_APP", default="giga_chat.api.main:app")
